...
class RasterizedTNTStageOne(nn.Module):
...
#        self.fc = nn.Linear(in_features=2048, out_features=num_targets)
        ...
        self.motion_network = MLP(in_channels=(2048), out_channels=3, hidden_unit=num_mlp_hidden)
...
#        if self.training:
        if True:
            ...
            stage1_goal = self.motion_network(embedding)
  

            loss = 0
            # [batch_size, num_steps * 2]
            targets = (torch.cat((data_batch["target_positions"][:,-1,:2], data_batch["target_yaws"][:,-1,:]), dim=1)).view(
                batch_size, -1
            )
