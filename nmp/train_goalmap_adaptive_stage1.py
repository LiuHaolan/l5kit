...
from model.goal_map import RasterizedGaussianGoalMapWithUNetStage1

model = RasterizedGaussianGoalMapWithUNetStage1(
        model_arch="unet",
        num_input_channels=rasterizer.num_channels(),
        num_targets=3 * cfg["model_params"]["future_num_frames"],  # X, Y, Yaw * number of future states,
        weights_scaling= [1., 1., 1.],
...
for epochs in range(epoch_num):
    ...
    # write a loop in evaluation dataset
            ...
            if cfg["debug"] == False:
                torch.save(model.state_dict(),"./gaussian/stage1/planning_goalmap_balanced_{}_{}.pt".format(epochs, step))
