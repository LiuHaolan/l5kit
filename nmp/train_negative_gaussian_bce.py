...
preprocessed_dir = "/mnt/scratch/v_liuhaolan/mask"


from model.goal_map import RasterizedGaussianGoalMapWithUNetGridBCE

downweight = 0.4

#print("downweight: {}".format(downweight))

model = RasterizedGaussianGoalMapWithUNetGridBCE(
        model_arch="unet",
...
#        motion_loss = np.round(motion_loss.item(),7)
    ...
    # write a loop in evaluation dataset
            ...
            if cfg["debug"] == False:
                torch.save(model.state_dict(),"./gaussian/new/planning_{}_{}_bce.pt".format(epochs, step))
