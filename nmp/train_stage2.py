...
from model.model_stage2 import RasterizedTNTStageTwo
model = RasterizedTNTStageTwo(
        model_arch="resnet50",
...
for epochs in range(epoch_num):
    ...
    for step,batch in enumerate(iter_bar):
        ...
        result = model(batch, goal_batch=None)
        loss = result["loss"]
        # Backward pass
        optimizer.zero_grad()
        loss.backward()
...
#        iter_bar.set_description(f"loss: {(loss_v)} motion_loss: {motion_loss} target_loss: {target_loss} classification: {cl_loss} regression loss: {rg_loss}")
    ...
    for step,batch in enumerate(val_dataloader):
        ...
        result = model(batch,goal_batch = gt_goal_batch)
        loss = result["loss"]
    ...
    torch.save(model.state_dict(),"../dataset_gen/planning_stage2_{}.pt".format(epochs))
