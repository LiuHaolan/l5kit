...
# get config
cfg = load_config_data("./consec.yaml")

# rasterisation and perturbation
rasterizer = build_rasterizer(cfg, dm)

#preprocessed_dir = "/mnt/scratch/v_liuhaolan/preprocessed_goal"
preprocessed_dir = "/mnt/scratch/v_liuhaolan/val_preprocessed" #preprocessed_128"


from l5kit.dataset import CachedEgoDataset

train_zarr = ChunkedDataset(dm.require(cfg["train_data_loader"]["key"])).open()
train_dataset = EgoDataset(cfg, train_zarr, rasterizer)#, preprocessed_path=preprocessed_dir)
...
model = RasterizedGaussianGoalMapWithUNetGrid(
        model_arch="unet",
        num_input_channels=rasterizer.num_channels(),
        num_targets=3 * cfg["model_params"]["future_num_frames"],  # X, Y, Yaw * number of future states,
        weights_scaling= [1., 1., 1.],
...
model_path="/mnt/home/v_liuhaolan/haolan/l5kit/nmp/gaussian/unet/planning_goalmap_balanced_6_0.pt"
...
#        motion_loss = np.round(motion_loss.item(),7)
    ...
    # write a loop in evaluation dataset
            ...
            if cfg["debug"] == False:
                torch.save(model.state_dict(),"./gaussian/optimized/planning_goalmap_balanced_{}_{}.pt".format(epochs, step))
