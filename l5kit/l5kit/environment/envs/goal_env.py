...
from l5kit.configs.custom_conf import SAMPLED_GOAL_NUM
...
class GoalEnv(gym.Env):
    ...
    def __init__(self, env_config_path: Optional[str] = None, dmg: Optional[LocalDataManager] = None,
        ...
        self.action_space = spaces.Discrete(SAMPLED_GOAL_NUM)
        ...
        # read predefined anchors
        import pickle
        
        f = open('/mnt/home/v_liuhaolan/haolan/l5kit/nmp/anchor.pkl', 'rb')
        picklelist = pickle.load(f)
        self.traj_anchor = pickle.loads(picklelist[0])
        self.yaw_anchor = pickle.loads(picklelist[1])
    ...
    def step(self, action: int) -> GymStepOutput:
        reward = 0
        for idx in range(20):
            obs, r, done, info = self.step_idx(action, idx)
            reward = reward + r
        return obs, reward, done, info

    def step_idx(self, action: int, idx: int) -> GymStepOutput:
        ...
        if not self.sim_cfg.use_ego_gt:
            ...
            ego_output = self._convert_action_to_ego_output(action, idx)
    ...
    def _convert_action_to_ego_output(self, action: int, idx: int) -> Dict[str, np.ndarray]:
        ...
        """
        # the action formatarray : [batch_size, action_id]
        """
        if self.use_kinematic:
        ...
        return data_dict
        """
        pred_positions = self.traj_anchor[action][idx]
        pred_yaws = self.yaw_anchor[action][idx]
        #print(pred_positions.shape)
        pred_positions = pred_positions.reshape(1,1,2)
        pred_yaws = pred_yaws.reshape(1,1,1)
        data_dict = {"positions": pred_positions, "yaws": pred_yaws}
        return data_dict
