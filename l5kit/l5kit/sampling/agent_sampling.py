...
def generate_agent_sample(
    ...
    if not augmented:
        ...
        import random
        from math import pi
        shift = random.uniform(-pi/2,pi/2)

        input_im = rasterizer.augment_rasterize(history_frames, history_agents, history_tl_faces, selected_agent, random_shift=shift)
    # print("rasterization time: {}".format(time.time()-start))

    world_from_agent = compute_agent_pose(agent_centroid_m, agent_yaw_rad)
    agent_from_world = np.linalg.inv(world_from_agent)
    
    raster_from_world = render_context.raster_from_world(agent_centroid_m, agent_yaw_rad+shift)

    future_positions_m, future_yaws_rad, future_extents, future_availabilities = get_relative_poses(
        future_num_frames, future_frames, selected_track_id, future_agents, agent_from_world, agent_yaw_rad+shift,
    )
    ...
    history_positions_m, history_yaws_rad, history_extents, history_availabilities = get_relative_poses(
        history_num_frames + 1, history_frames, selected_track_id, history_agents, agent_from_world, agent_yaw_rad+shift,
    )
...
def extract_agent(rasterizer, history_frames: np.ndarray,
    ...
    raster_from_world = rasterizer.render_context.raster_from_world(ego_translation_m, ego_yaw_rad)

    # this ensures we always end up with fixed size arrays, +1 is because current time is also in the history
    out_shape = (rasterizer.raster_size[1], rasterizer.raster_size[0], rasterizer.history_num_frames + 1)
...
def generate_agent_sample_pseudolabel(
        state_index: int,
        frames: np.ndarray,
        agents: np.ndarray,
        tl_faces: np.ndarray,
    ...
    #input_im = rasterizer.rasterize(history_frames, history_agents, history_tl_faces, selected_agent)
    res_agents = extract_agent(rasterizer, history_frames, history_agents, history_tl_faces, selected_agent)
